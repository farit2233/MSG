                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered mb-4 small-table">
                                        ...
                                        <tfoot>
                                            ...
                                            $cart_promotions = [];         // Array สำหรับเก็บข้อมูลโปรโมชั่นทั้งหมดที่พบในตะกร้า
                                            $product_has_promo_status = []; // Array สำหรับเช็คว่าสินค้าแต่ละชิ้นมีโปรโมชั่นหรือไม่ (true/false)
                                        ...
                                        WHERE pp.product_id = {$item['product_id']} AND pp.status = 1 AND pp.delete_flag = 0";
                                                ...
                                                if ($promo_result && $promo_result->num_rows > 0) {
                                                    ...
                                                    // เก็บข้อมูลโปรโมชั่น โดยใช้ promotion_id เป็น key เพื่อไม่ให้ซ้ำซ้อน
                                                    $cart_promotions[$promo_data['id']] = $promo_data;
                                                    // บันทึกว่าสินค้านี้มีโปรโมชั่น (id ของโปรโมชั่น)
                                                    $product_has_promo_status[] = $promo_data['id'];
                                                } else {
                                                    // บันทึกว่าสินค้านี้ "ไม่มี" โปรโมชั่น
                                                    $product_has_promo_status[] = false;
                                                }
                                            }

                                            // --- วิเคราะห์โปรโมชั่นที่รวบรวมได้ ---
                                            $is_promo_applicable = false; // ตัวแปรตั้งต้นว่าโปรโมชั่น "ยังใช้ไม่ได้"

                                            // กรองเอาเฉพาะ ID โปรโมชั่นที่ไม่ซ้ำกัน และไม่นับค่า false
                                            $unique_promo_ids = array_unique(array_filter($product_has_promo_status));

                                            // เงื่อนไขในการใช้โปรโมชั่นได้คือ:
                                            // 1. ต้องมีโปรโมชั่นในตะกร้า (count > 0)
                                            // 2. ต้องมีโปรโมชั่นแค่ "ชนิดเดียว" เท่านั้น (count == 1)
                                            // 3. ต้องไม่มีสินค้าชิ้นไหนที่ "ไม่มี" โปรโมชั่นเลย (in_array(false, ...) === false)
                                            ...
                                            <?php
                                            // --- แสดงผลโปรโมชั่นถ้ามีในตะกร้า ---
                                            if (!empty($cart_promotions)) :

                                                // กำหนด class สำหรับทำให้ตัวจาง ถ้าใช้โปรโมชั่นไม่ได้
