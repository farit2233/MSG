				...
				<div class="card-body">
					<div class="form-group">
						...
						<div class="custom-file">
							<input type="file" class="custom-file-input" name="img" id="img" onchange="displayImg(this)" <?= !isset($id) ? 'required' : '' ?>>
...
<script>
	...
	$(document).ready(function() {
		...
		$('#product-form').submit(function(e) {
			...
			// ▼▼▼ โค้ดที่เพิ่มใหม่: ตรวจสอบรูปภาพหลักสำหรับสินค้าใหม่ ▼▼▼
			const productId = $('input[name="id"]').val();
			const mainImageFile = $('#img')[0].files[0];

			// ตรวจสอบ: ถ้าเป็นสินค้าใหม่ (id ว่าง) และยังไม่ได้เลือกรูปภาพหลัก
			if (!productId && !mainImageFile) {
				Swal.fire({
					icon: 'error',
					title: 'กรุณาอัปโหลดรูปภาพสินค้า',
					text: 'คุณต้องอัปโหลดรูปภาพสินค้าหลักสำหรับสินค้าใหม่',
				});
				return; // หยุดการทำงานทันที ไม่ส่งฟอร์ม
			}
			...
			$(this).find('input, select, textarea').not('input[type=file]').each(function() {
				...
				// ▼▼▼ นี่คือจุดที่แก้ให้หายขาด ▼▼▼
				// ถ้าช่องนั้นไม่มี 'name' attribute (เป็น undefined) ให้ข้ามไปเลย
				if (typeof name === 'undefined' || name === false) {
					return; // เทียบเท่ากับ 'continue' ใน .each()
				}
				// ▲▲▲ สิ้นสุดการแก้ไข ▲▲▲
			...
			$.ajax({
				...
				data: formData, // 🚨 แก้ไขเป็นตัวแปรนี้
				cache: false,
				contentType: false,
				processData: false,
				method: 'POST',
